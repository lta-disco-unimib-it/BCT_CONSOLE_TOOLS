...
public class IOTracesIndexEditor extends EditorPart implements IFindActionEditorSupport {

	private URI uri;
	private List<Trace> ioTraces;
	private VirtualTable<Trace> table;
	...
	public void init(IEditorSite site, IEditorInput input) throws PartInitException {
		...
		if (input instanceof FileStoreEditorInput) {
			...
			if(URIUtil.rawTraceURI(uri))
				super.setPartName("Raw IO Traces Index");
			else
				super.setPartName("Normalized IO Traces Index");
			loadIOTraces();
		} else {
			throw new PartInitException("IORawTracesEditor input must be a FileStoreEditorInput instance!");
		}
	}
	...
	public void createPartControl(Composite parent) {
		table = new VirtualTable<Trace>(parent, new String[] {"Method signature"});
		table.setMonitoringConfiguration(mc);
		table.setContent(ioTraces);
	}
	...
	private void loadIOTraces() {
		...
		try {
			ioTraces = URIMapperFactory.getMapper(mc).getIOTraces(uri);
