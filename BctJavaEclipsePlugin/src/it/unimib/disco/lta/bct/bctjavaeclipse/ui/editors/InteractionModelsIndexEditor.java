...
public class InteractionModelsIndexEditor extends EditorPart implements IFindActionEditorSupport {

	private URI uri;
	private VirtualTable<FSAModel> table;
	private MonitoringConfiguration mc;
	private List<FSAModel> interactionModels;
	...
	public void init(IEditorSite site, IEditorInput input) throws PartInitException {
		...
		if (input instanceof FileStoreEditorInput) {
			...
			uri = fsInput.getURI();
			super.setPartName("FSA Models Index");
	...
	public void createPartControl(Composite parent) {
		table = new VirtualTable<FSAModel>(parent, new String[] {"Method signature"});
		table.setMonitoringConfiguration(mc);
		table.setContent(interactionModels);
	}
	...
	private void loadInteractionModels() {
		mc = MonitoringConfigurationRegistry.getInstance().getMonitoringConfiguration(uri);
		
		try {
			interactionModels = URIMapperFactory.getMapper(mc).getFSAModels(uri);
