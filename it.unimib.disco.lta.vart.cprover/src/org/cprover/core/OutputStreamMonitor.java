...
public class OutputStreamMonitor implements IFlushableStreamMonitor {
    ...
    private void fireStreamAppended(String text) {
        getContentNotifier().notifyAppend(text);
    }

    /**
     * Notifies the listeners that the stream has
     * been closed.
     */
    private void fireStreamClosed() {
        getClosedNotifier().notifyClosed();
    }
    ...
    private ContentNotifier getContentNotifier() {
    ...
    private ClosedNotifier getClosedNotifier() {
        return new ClosedNotifier();
    }
    
    class ClosedNotifier implements ISafeRunnable {
        
        private IClosingStreamListener fListener;
        ...
        public void notifyClosed() {
            Object[] copiedListeners= fListeners.getListeners();
            for (int i= 0; i < copiedListeners.length; i++) {
                if (copiedListeners[i] instanceof IClosingStreamListener) {
                    fListener = (IClosingStreamListener) copiedListeners[i];
                    SafeRunner.run(this);
                }
            }
            fListener = null;
