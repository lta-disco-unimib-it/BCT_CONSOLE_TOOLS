 ...
 * @since 3.0
 */
public class CProverProcess extends PlatformObject implements IProcess {
	 ...
	 */
	public CProverProcess(ILaunch launch, Process process, String name, Map attributes) {
		setLaunch(launch);
	...
	protected void fireTerminateEvent() {
		//System.out.println("fireTerminatedEvent() at P: " + this.toString());		
	...
	static class ProcessMonitorThread extends Thread {
		 ...
		 */
		protected CProverProcess fRuntimeProcess;
		...
		public void run() {
			...
			while (fOSProcess != null) {
				try {
					...
					//System.out.println("monitor.run null fOSProcess-" + fOSProcess.toString() + ", terminate fRuntimeProcess-" + fRuntimeProcess.toString());
		 ...
		 */
		public ProcessMonitorThread(CProverProcess process) {
			super(DebugCoreMessages.ProcessMonitorJob_0); 
			setDaemon(true);
			fRuntimeProcess= process;
			fOSProcess= process.getSystemProcess();
	       // System.out.println("Initialize monitor:" + fOSProcess.toString() + " " + fRuntimeProcess.toString());
		}
